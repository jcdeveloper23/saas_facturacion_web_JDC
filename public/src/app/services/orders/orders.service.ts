...
export class OrdersService {
  ...
  public getOrdersByStudent(student_id: string) {
    return this.db.collection('students')
      .doc(`${student_id}`)
      .collection('orders', ref => ref.where('order_state', '==', false)
        .where('order_state_payment_method', '==', true)
      )
      .valueChanges();
  }
  public getOrdersByProviderInStatusFalse(provider_id: string, dateSelected: string) {
    if (dateSelected == null) {
      var dateCurrent = new Date();
      dateSelected = dateCurrent.getFullYear() + '-' +
  ...
  public getAllOrdersConfirmationPending(provider_id: string, value: string) {
    return this.db.collection('providers')
      .doc(`${provider_id}`)
      .collection('orders', ref => ref
        .where('order_state_payment_method', '==', false)
        .where('order_state_payment_method_string', '==', value)
        .orderBy('order_state_payment_method_string')  // ✅ Primero el campo con desigualdad
        .orderBy('order_transaccion_id', 'desc')       // ✅ Luego el campo adicional
      )
      .valueChanges();
  }
