...
export class RepresentativeComponent implements OnInit {
  ...
  @ViewChild('tableRepresentativesSuperAdmin') paginatorListSuperAdmin: MatPaginator;
  public dataSourceListSuperAdmin: MatTableDataSource<Representative>;
  ...
  public arrayStudentsAux: Array<boolean> = [];
  ...
  public getRepresentativesList() {
    if (this.infoUser.users_account_type !== '0') {
      ...
      this.representativeService.getAllRepresentatives().pipe(take(1)).subscribe((representatives) => {
        this.array_representative = representatives;
        this.dataSourceListSuperAdmin = new MatTableDataSource<Representative>(representatives);
  ...
  public newRepresentative() {
    this.representative = {};
    this.arrayStudents = [];
    this.arrayStudentsAux = []
    this.isEdit = false;
    this.representative.representative_state = false;
    this.representative.representative_state_confirm = false;
  ...
  public saveStudent(student: Student, isValid: boolean, form: NgForm, i: number) {
    if (isValid === true) {
      this.isValidFormStudent.push(true);
      this.arrayStudentsAux[i] = true;
    } else {
      this.isValidFormStudent.splice(1, 1);
      this.arrayStudentsAux[i] = false;
    }
  ...
  public addStudent() {
    if (this.representative.representative_identification !== null && this.representative.representative_identification !== undefined && this.representative.representative_identification !== '') {
      this.arrayStudentsAux.push(true);
      this.arrayStudents.push({
        student_state_register: true,
        student_state: true,
  ...
  public deleteStudent(i: number) {
    if (!this.isEdit) {
      ...
      const student: Student = this.arrayStudents[i];
      const studentValue = this.studentService.getStudentById(student).pipe(take(1)).toPromise();
      if (studentValue) {
  ...
  public async sendEmail(representative: Representative) {
    this.representativeService.getRepresentativeIdSendEmail(representative.representative_id).pipe(take(1)).subscribe((rep: Representative) => {
      if (rep) {
        if (!rep.representative_send_email) {
          rep.representative_send_email = 0;
        } else {
  ...
  public editRepresentative(representative: Representative) {
    ...
    this.userService.getUserByEmail(this.representative.representative_email).pipe(take(1)).subscribe((user) => {
      this.representativeUser = user[0];
    });

    this.studentService.getStudentsByRepresentative(representative.representative_id).pipe(take(1)).subscribe((students) => {
      this.selectStudent(students[0], 0)
