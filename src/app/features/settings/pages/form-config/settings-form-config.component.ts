...
import {
  ...
  SpinnerModule, AlertModule, TooltipModule, FormModule, CalloutComponent
} from '@coreui/angular';
...
export interface Section {
  ...
  label:         string;
  icon:          string;
  activeRows:    FieldRow[];   // visible = true
  availableRows: FieldRow[];   // visible = false, not alwaysVisible
}
...
@Component({
  selector: 'app-settings-form-config',
  standalone: true,
  imports: [
    CommonModule, FormsModule,
    CardModule, ButtonModule, GridModule, BadgeModule,
    SpinnerModule, AlertModule, TooltipModule, FormModule, CalloutComponent, IconModule
  ],
  templateUrl: './settings-form-config.component.html'
})
export class SettingsFormConfigComponent implements OnInit {
  ...
  private loadConfig(): void {
    ...
    this.configSvc.getConfig(this.tenantSvc.companyId, this.entity()).then(cfg => {
  ...
  async save(silent = false): Promise<void> {
    ...
    try {
      await this.configSvc.saveConfig(this.tenantSvc.companyId, this.entity(), this.rawConfig());
