 ...
 */
exports.uploadCertificate = (0, https_1.onCall)(async (request) => {
    // ── Auth check ────────────────────────────────────────────────────────────
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'Debe estar autenticado para subir un certificado.');
    }
    ...
    if (!data.companyId || typeof data.companyId !== 'string') {
        throw new https_1.HttpsError('invalid-argument', 'companyId es requerido.');
    }
    if (!data.certificateBase64 || typeof data.certificateBase64 !== 'string') {
        throw new https_1.HttpsError('invalid-argument', 'certificateBase64 es requerido.');
    }
    if (typeof data.password !== 'string') {
        throw new https_1.HttpsError('invalid-argument', 'password es requerido.');
    }
    // ── Permission check ──────────────────────────────────────────────────────
    const isSuperAdmin = callerRole === 'super_admin';
    const isCompanyAdmin = (callerRole === 'admin') && callerCompanyId === data.companyId;
    if (!isSuperAdmin && !isCompanyAdmin) {
        throw new https_1.HttpsError('permission-denied', 'Solo un administrador de la empresa o super_admin puede subir el certificado.');
    }
    ...
    try {
        ...
        throw new https_1.HttpsError('invalid-argument', 'Certificado inválido o contraseña incorrecta.');
    }
    // ── Extract X.509 certificate ─────────────────────────────────────────────
    const certBags = p12.getBags({ bagType: forge.pki.oids.certBag });
    const certBag = certBags[forge.pki.oids.certBag]?.[0];
    if (!certBag?.cert) {
        throw new https_1.HttpsError('invalid-argument', 'No se encontró certificado en el archivo .p12.');
    }
    ...
    const now = new Date();
    if (expiryDate < now) {
        throw new https_1.HttpsError('invalid-argument', `El certificado expiró el ${expiryDate.toLocaleDateString('es-EC')}.`);
    }
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', 'Error al guardar el certificado en Storage.');
    }
    // ── Update Firestore ───────────────────────────────────────────────────────
    try {
        ...
        throw new https_1.HttpsError('internal', 'Error al actualizar los datos del certificado.');
    }
    // ── Return result (never include .p12 or password) ────────────────────────
    const expiresIn = Math.floor((expiryDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
    return {
        success: true,
        thumbprint,
        subject,
